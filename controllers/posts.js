...
export const getPosts = (req, res)=>{
  ...
  const sql = "SELECT DISTINCT p.*, u.firstName, u.userName, u.profilePicture FROM Posts as p JOIN users AS u ON (u.id = p.userId) LEFT JOIN Follow AS f ON ( p.userId = f.followed OR p.userId = f.follower  ) WHERE f.follower = ? OR f.followBack = ? OR p.userId = ? ORDER BY p.createdAt DESC;"; // 
